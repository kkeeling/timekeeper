...
FALLBACK_MAX_HOURS = 40.0  # Maximum daily hours cap
...
- Be realistic with time estimates"""
    ...
    try:
        ...
        # Round to nearest half hour and cap total_hours at 4
        result_dict['total_hours'] = min(round_to_half_hour(result_dict['total_hours']), 4)
        for task in result_dict.get('major_tasks', []):
            task['hours'] = round_to_half_hour(task['hours'])
        ...
        hours = min(round_to_half_hour(FALLBACK_BASE_HOURS + total_lines / FALLBACK_LINES_PER_HOUR), 4)
